...
def kmeansPlusPlus(normalizedData,k=1,maxIter=50):
    rng = np.random.default_rng()
    numPoints = np.shape(normalizedData)[0]
    centroids = [normalizedData[rng.integers(0,numPoints),:]]
    
    for i in range(k-1):
        distances = [min([np.linalg.norm(j-h) **2 for h in centroids]) for j in normalizedData]
        normalizedDistances = distances/sum(distances)
        cutoffs = np.cumsum(normalizedDistances)
        centroids.append(normalizedData[np.argmax(cutoffs > rng.random())])
...
for label in unique_labels:
    ...
    ax1.scatter(cluster_points[:, 0], cluster_points[:, 1], s=10, color=color, label=label_name)

ax1.set_title("DBSCAN Clustering")
ax1.set_xlabel("X")
ax1.set_ylabel("Y")
ax1.legend()
plt.show  # Save the figure as a PNG file


# K-means clustering
normalizedData = normalizeData(data, method='none')
k = 6
centroids, clusterInds = kMeans(normalizedData, k, maxIter=20)
centroids, clusterInds = kmeansPlusPlus(normalizedData,k ,maxIter=50)
